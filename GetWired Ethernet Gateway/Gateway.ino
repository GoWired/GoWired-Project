...
bool VerifyControllerUplink = true;
uint32_t TIME_1 = 0;
...
void before() {
  ...
  pinMode(CONF_BUTTON, INPUT_PULLUP); digitalWrite(BUS_RELAY, 0);
  pinMode(BUS_RELAY, OUTPUT); digitalWrite(CONF_LED, 0);
...
void loop() {
  ...
  #ifdef ENABLE_WATCHDOG
    if((millis() > LastUpdate + CONNECTION_CHECK_INTERVAL) && VerifyControllerUplink) {
      if(!requestTime())  {
        delay(10000);
      }
      LastUpdate = millis();
  ...
  if(!digitalRead(CONF_BUTTON) && ButtonHigh) {
    ...
    while(!digitalRead(CONF_BUTTON)) {
      ...
      if(millis() - TIME_1 > 500) {
        ...
        VerifyControllerUplink = ButtonState == 1 ? false : true;
